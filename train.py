...
from le_pde_uq.models import get_model, load_model, unittest_model, build_optimizer, test, rollout
from le_pde_uq.pytorch_net.util import pload, pdump, Printer, get_time, init_args, update_args, clip_grad, Batch, set_seed, update_dict, filter_kwargs, plot_vectors, plot_matrices, make_dir, get_pdict, to_np_array, record_data, make_dir, Early_Stopping, str2bool, get_filename_short, print_banner, get_num_params, ddeepcopy as deepcopy, write_to_config
from le_pde_uq.utils import get_model_dict, EXP_PATH, get_elements, is_diagnose, get_keys_values, loss_op, to_tuple_shape, parse_string_idx_to_list, parse_multi_step, get_device, Channel_Gen, process_data_for_CNN, get_activation, get_normalization, Mean, Flatten, Permute, Reshape, to_cpu
...
if "discriminator" in kwargs:
    n_params_disc = get_num_params(kwargs["discriminator"])
    data_record["n_params_disc"] = n_params_disc
    p.print("\tn_params_disc: {}".format(n_params_disc))
if "ebm" in kwargs:
    n_params_ebm = get_num_params(kwargs["ebm"])
    data_record["n_params_ebm"] = n_params_ebm
    p.print("\tn_params_ebm: {}".format(n_params_ebm))
else:
    print()
...
while epoch < args.epochs:
    ...
    p.print(filename)
    p.print("Epoch {:03d}:     Train: {:.4e}     total_loss: {:.4e}     count: {:.4e}".format(
        epoch + 1, train_loss, total_loss, count), end="")
    ...
    if epoch % args.test_interval == 0 or epoch == args.epochs - 1:
        ...
        p.print("       Val: {:.4e}     Test: {:.4e}".format(val_loss, test_loss), end="")
        for key, loss_ele in test_info.items():
            #p.print("               {}: ({:.4e}, {:.4e})".format(key.split("loss_")[-1], loss_ele/100, loss_ele/100), end="")
            p.print("               {}: ({:.4e}, {})".format(key.split("loss_")[-1], loss_ele, "{:.4e}".format(test_info[key]) if key in test_info else None), end="")  
    print()
    ...
    if epoch % args.save_interval == 0 and epoch >= 0:
        ...
        if args.disc_coef > 0 or args.disc_coef == -1:
            record_data(data_record, [kwargs["discriminator"].model_dict], ["disc_model_dict"])
        if args.is_ebm:
            record_data(data_record, [kwargs["ebm"].model_dict], ["ebm_model_dict"])
        with open(filename, "wb") as f:
            pickle.dump(data_record, f)
    if val_loss < best_val_loss:
        ...
        if args.disc_coef > 0 or args.disc_coef == -1:
            data_record["best_disc_model_dict"] = kwargs["discriminator"].model_dict
        if args.is_ebm:
            data_record["best_ebm_model_dict"] = kwargs["ebm"].model_dict
    if True:
